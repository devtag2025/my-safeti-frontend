...
export default function EditReportForm({ reportToEdit, onSuccess, onCancel }) {
  ...
  // Populate form with reportToEdit data
  useEffect(() => {
    if (reportToEdit) {
      // Set basic fields
      setValue("name", reportToEdit.name || "");
      setValue("email", reportToEdit.email || "");
      setValue("phone", reportToEdit.phone || "");
      setValue(
        "date",
        reportToEdit.date ? reportToEdit.date.split("T")[0] : ""
      );
      setValue("time", reportToEdit.time || "");
      setValue("location", reportToEdit.location || "");
      setValue("streetNumber", reportToEdit.streetNumber || "");
      setValue("crossStreet", reportToEdit.crossStreet || "");
      setValue("suburb", reportToEdit.suburb || "");
      setValue("state", reportToEdit.state || "NSW");
      setValue("incidentType", reportToEdit.incidentType || "Collision");
      setValue("vehicleType", reportToEdit.vehicleType || "Car");
      setValue("description", reportToEdit.description || "");
      setValue("hasDashcam", reportToEdit.hasDashcam || false);
      setValue("hasAudio", reportToEdit.hasAudio || false);
      setValue("canProvideFootage", reportToEdit.canProvideFootage || false);
      setValue("acceptTerms", reportToEdit.acceptTerms || false);

      // Handle vehicles
      if (reportToEdit.vehicles && reportToEdit.vehicles.length > 0) {
        // Format vehicles to match form structure
        const formattedVehicles = reportToEdit.vehicles.map((vehicle) => ({
          registration: vehicle.registration || "",
          registrationState: vehicle.registrationState || "NSW",
          make: vehicle.make || "",
          model: vehicle.model || "",
          vehicleColour: vehicle.vehicleColour || "",
          bodyType: vehicle.bodyType || "Sedan",
          identifyingFeatures: vehicle.identifyingFeatures || "",
          isRegistrationVisible: vehicle.isRegistrationVisible || "Unknown",
          _id: vehicle._id, // Keep the ID for reference
        }));

        replace(formattedVehicles);
      }
    }
  }, [reportToEdit, setValue, replace]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Update the report
      await updateReport(reportToEdit._id, reportToEdit.userId, data);
      setSuccess(true);
    ...
    <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="bg-gray-50 border-b px-6 py-4">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Update the information about the incident to ensure accuracy.
        </p>
      </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {success && (
            <div className="p-4 border border-green-200 bg-green-50 text-green-700 rounded-md">
              <strong>Success!</strong> Your report has been updated
              successfully!
            </div>
          )}
